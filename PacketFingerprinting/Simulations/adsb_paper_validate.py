...
def main(output_shape, i):
    ## Parameters
    input_shape=512
    fn_model_base = f'../Results/ADSB_Paper/ADSB_{output_shape}/Res_{i}/'
    
    filepath= f'../Results/ADSB_Paper_val/ADSB_{output_shape}_big/'
    ...
    with open(fn_model_base+'Results/info.json', "r") as outfile:
...
def test_model(model, ds, filepath,th):
    ...
    loss,corr,a,t,ia,it,flag= model.predict(ds.test())
    info = {
        'recall':{
            'precision': cal_precision(loss,flag==1,th),
            'recall': cal_recall(loss,flag==1,th),
            'th': th
        }}


    res = {'Loss':loss,'Corr':corr,'anchor':a,'test':t,'idx_a':ia,'idx_t':it,'f':flag}
